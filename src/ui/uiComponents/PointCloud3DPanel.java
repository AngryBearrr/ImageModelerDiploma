...
public class PointCloud3DPanel extends JPanel implements MouseWheelListener {
    ...
    public void setScaleFactor(double scaleFactor) { this.scaleFactor = scaleFactor; repaint(); }
    ...
    private void drawGrid(Graphics2D g2) {
        ...
        for(int x=-gridSize;x<=gridSize;x+=step) drawLine3D(g2,x,0,-gridSize, x,0,gridSize);
        for(int z=-gridSize;z<=gridSize;z+=step) drawLine3D(g2,-gridSize,0,z, gridSize,0,z);
    }
    ...
    private void drawPoints(Graphics2D g2) {
        ...
        for(int i=0;i<cloud.size();i++){
            ...
            double x=p3.getX()*scaleFactor, y=p3.getY()*scaleFactor, z=p3.getZ()*scaleFactor;
            Point p = project(x,y,z);
            boolean active = i==activeIndex;
            int sz = active? activeSize: baseSize;
            g2.setColor(active? Color.YELLOW: Color.WHITE);
            g2.fillOval(p.x-sz/2, p.y-sz/2, sz, sz);
            g2.setColor(active? Color.YELLOW: Color.CYAN);
            g2.drawString(p3.getName(), p.x+sz, p.y-sz);
        }
    }
    ...
    private Point project(double x,double y,double z){
        ...
        double depth=z2+cameraDistance; if(depth<1e-3) depth=1e-3;
    ...
    @Override public void mouseWheelMoved(MouseWheelEvent e){
        ...
        if(n<0) zoom *= Math.pow(ZOOM_STEP,-n);
        else    zoom /= Math.pow(ZOOM_STEP, n);
